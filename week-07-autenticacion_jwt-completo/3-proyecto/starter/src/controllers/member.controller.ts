...
function handleZod(err: unknown, next: NextFunction): boolean {
  if (err instanceof ZodError) {
    next(new AppError(422, err.issues.map((i) => i.message).join('; ')));
    return true;
  }
  return false;
}
...
export async function getMemberById(req: Request<{ id: string }>, res: Response, next: NextFunction) {
  try {
...
export async function updateMember(req: Request<{ id: string }>, res: Response, next: NextFunction) {
  try {
    ...
    const member = await memberService.update(req.params.id, body);
    if (!member) throw new AppError(404, 'Member not found');
